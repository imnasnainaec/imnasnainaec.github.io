  ...
  if (n >= 0) {
    ...
    for (i = 0; i < 7; i++) {
      ...
      for (j = 0, len = clock[i].length; j < len; j++) {
        m = clock[i][j];
        code +=
          "<li><div class='link'><a href='javascript:load(" +
          m +
          ")'>" +
          props[m][0] +
          "</a><div class='popup'>";
        ...
        if (i === 0) {
          code +=
            "&#9734; &hArr; &#8226; : " + reason[imp[1]] + citationCode(imp[2]);
        } else if (imp[0] < -1) {
          code +=
            "Mutually exclusive: " + reason[imp[1]] + citationCode(imp[2]);
        } else {
          code += "&#9734; " + rel1[i] + " &#8226; : " + reason[imp[1]];
          if (i > 4) {
            code += counterexampleCode(n, m);
          }
...
function counterexampleCode(n, m) {
  ...
  if (EX != null) {
    ...
    if (hogID > 0) {
      snippet +=
        " counterexample <a href='http://hog.grinvin.org/ViewGraphInfo.action?id=" +
        hogID +
        "' target='_blank' rel='noopener noreferrer'>" +
        EX +
        "<span class='popup'><br><embed src='http://hog.grinvin.org/GraphImage.action?id=" +
        hogID +
        "&size=80' /></span></a>";
    } else {
...
function propLister(items) {
  ...
  for (i = 0, len = items.length; i < len; i++) {
    ...
    if (r == propID) {
      ...
      snippet +=
        "<li><a href='javascript:load(" + r + ")'>" + props[r][0] + "</a></li>";
    }
  }
  return snippet;
}
